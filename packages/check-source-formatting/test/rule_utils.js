...
describe('Rule Utils', function() {
	...
	it('should get constants from node properly', function(done) {
		parse(
	...
	it('should calculate line distance', function(done) {
		var varStr = [
			'var a = 1;',
			'var b = 2;',
			'var c = 3;',
		...
		parse(varStr, function(node) {
			if (node.type === 'Program') {
				var variables = _.filter(node.body, {
	...
	it('should sort array naturally', function() {
		var naturalCompare = ruleUtils.naturalCompare;

		assert.equal(
			naturalCompare('a', 'a', false),
			0,
		...
		assert.equal(
			naturalCompare('A', 'a', false),
			-1,
			'A should not come before a'
		);
		assert.equal(
			naturalCompare('a', 'b', false),
			-1,
			'a should come before b'
		);
		assert.equal(
			naturalCompare('iStragedy', 'isTragedy', false),
			-1,
			'iStragedy should come before isTragedy'
		);
		...
		assert.equal(
			naturalCompare('2', '1'),
			1,
			'"2" should not come before "1"'
		);

		assert.equal(
			naturalCompare('a', 'a', true),
			0,
			'a should be the same as a'
		);
		assert.equal(
		...
		assert.equal(
			naturalCompare('a', 'b', true),
			-1,
			'a should come before b'
		);
		assert.equal(
			naturalCompare('iStragedy', 'isTragedy', true),
			0,
			'iStragedy should be the same as isTragedy'
		);
	});
});
