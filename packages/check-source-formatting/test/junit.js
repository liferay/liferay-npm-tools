...
describe('JUnit reporting', function() {
	'use strict';
	...
	it('should generate a JUnit report', function() {
		...
		sinon
			.stub(fs, 'readFile')
			.callsFake(function(path, encoding, callback) {
		...
		sinon
			.stub(fs, 'writeFile')
			.callsFake(function(path, content, callback) {
		...
		return reporter.generate().then(function(results) {
			...
			assert.equal(
				results,
				fs.readFileSync(
					path.join(__dirname, 'fixture', 'result.xml'),
					'utf-8'
				),
				'The result should match what we expect'
			);
		});
	});

	it('should generate a valid JUnit report', function() {
		var logger = new Logger.constructor();

		logger.log(1, 'Content is not valid', 'foo.js');
		logger.log(
			38,
			'Missing space between selector and bracket: &.no-title .asset-user-actions{',
			'xmlentity.css',
			'error'
		);
		...
		logger.log(
			141,
			'Sort attribute values: javascript:�0�removeGroup(',
			'unicode.css',
			'error'
		);
	...
	it('should generate a JUnit report to a custom path', function(done) {
		sinon.stub(Logger);
