...
describe('Formatter.CSS', function() {
	...
	it('should detect lower case hex codes', function() {
		return testFile(getFilePath('hex_lower_case.css')).spread(function(
			errors
		) {
	...
	it('should detect redundant hex codes', function() {
		return testFile(getFilePath('hex_redundant.css')).spread(function(
			errors
		) {
			assert.equal(errors.length, 1);
	...
	it('should detect invalid border resets', function() {
	...
	it('should detect invalid formatting in property rules', function() {
		return testFile(getFilePath('invalid_format.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should detect missing integers', function() {
		return testFile(getFilePath('missing_integer.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should detect missing spaces in list values', function() {
	...
	it('should detect missing newlines', function() {
		return testFile(getFilePath('missing_newlines.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should detect missing spaces in selectors', function() {
	...
	it('should detect needless quotes', function() {
		return testFile(getFilePath('needless_quotes.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should detect needless units', function() {
		return testFile(getFilePath('needless_unit.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should detect property sort', function() {
		return testFile(getFilePath('property_sort.css')).spread(function(
			errors,
			contents,
			newContents
		) {
			assert.startsWith(
				errors[0].msg,
				'Expected background to come before padding'
			);
		});
	});

	it('should detect trailing commas in selectors', function() {
		return testFile(getFilePath('trailing_comma.css')).spread(function(
			errors,
			contents,
			newContents
		) {
	...
	it('should use a custom lint log filter', function(done) {
		var testFilePath = path.join(
			__dirname,
			'fixture',
			'css',
			'at_rule_empty_line.css'
		);

		var source = fs.readFileSync(testFilePath, 'utf-8');

		var cssLoggerFilter = new Logger.constructor();
		var cssFormatterFilter = new Formatter.CSS(
			testFilePath,
			cssLoggerFilter
		);
...
describe('Formatter.CSS Excludes', function() {
	'use strict';
...
describe('Formatter.CSS Lint', function() {
	'use strict';
	...
	it('should find at least one lint error', function(done) {
		...
		Promise.resolve(response)
			.then(function(results) {
				...
				var hasLintError = _.some(lintConfig.rules, function(
					item,
					index
				) {
	...
	it('should be able to disable linting', function(done) {
		var testFilePath = path.join(
			__dirname,
			'fixture',
			'css',
		...
		Promise.resolve(result)
			.then(function(result) {
				...
				var hasRuleId = errors.some(function(item, index) {
					return !!item.ruleId;
				});
