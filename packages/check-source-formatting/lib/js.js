	...
	prototype: {
		...
		format(contents, lint) {
			...
			var newContents = iterateLines(
				...
				(content, index, collection) => {
					...
					context.nextItem =
						collection[lineNum] && collection[lineNum].trim();
		...
		_logLintResults(results) {
			...
			results.forEach((item, index) => {
				...
				instance.log(item.line, item.message, item.ruleId, {
					column: item.column,
					ruleId: item.ruleId
				});
			});
		...
		_processSyntax(contents) {
			...
			try {
				...
				this.log(
					e.lineNumber || 'n/a',
					sub(
						'Could not parse JavaScript: {0}',
						e.description || e.message
					)
				);

				this.logger.verboseDetails[this.file] = this._printAsSource(
					contents
				);
			}
		}
