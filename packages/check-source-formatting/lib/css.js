...
Formatter.CSS = Formatter.create({
	...
	prototype: {
		format(contents, lint) {
			...
			var newContents = iterateLines(
				...
				(item, index, collection) =>
					instance.processFile(item, index, collection, logger)
			);
			...
			return this._lint(contents, context)
				.then(results => {
		...
		formatPropertyItem(content) {
			return content
				.replace(REGEX.LEADING_SPACE, '')
				.replace(REGEX.LEADING_INCLUDE, '');
		},
		...
		processFile(content, index, collection, logger) {
			...
			if (re.hasProperty(content)) {
				var processed = this.processProperty(
					rawContent,
					content,
					context.nextItem,
					context
		...
		processProperty(rawContent, content, nextItem, context) {
			...
			var propertyContext = _.assign({}, context, {
				content
			});
		...
		_logLintResults(results) {
			...
			results[0].warnings.forEach((item, index) => {
				...
				instance.log(item.line, item.text, item.rule, {
					column: item.column,
					ruleId: item.rule
				});
			});
